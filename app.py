...
from models import *  # import SQLAlchemy tables
...
def venues():
  ...
  for areas in all_venues:
    area_venues = Venue.query.filter_by(state=areas.state).filter_by(city=areas.city).all()
    
    for venue in area_venues:
      venue_data.append({
        "id": venue.id,
        "name": venue.name, 
        "num_upcoming_shows": len(db.session.query(Show).filter(Show.venue_id==1).filter(Show.start_time > dt.now() ).all())
      })
    data.append({
      "city": areas.city,
      "state": areas.state, 
...
def search_venues():
  ...
  for result in search_result:
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": len(db.session.query(Show).filter(Show.venue_id == result.id).filter(Show.start_time > dt.now()).all())
    })
...
def show_venue(venue_id):
  ...
  upcoming_shows_query = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time >= dt.now()).all()
  upcoming_shows = []
...
def search_artists():
  ...
  for result in search_result:
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": len(db.session.query(Show).filter(Show.artist_id == result.id).filter(Show.start_time > dt.now()).all())
    })
...
def show_artist(artist_id):
  ...
  past_shows_query = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time>dt.now()).all()
  past_shows = []
  ...
  upcoming_shows_query = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time>dt.now()).all()
  upcoming_shows = []
